...
function CommentsPage() {
	...
	const guardarRespuesta = async (comentarioId) => {
		...
		try {
			const respuestaData = {
				idUser: user.idUser, // Asegúrate de que esta propiedad se llama idUser
				idComentario: comentarioId, // Asegúrate de que esta propiedad se llama idComentario
				text: respuestaText
			};
			...
			alert(response.data); // Mensaje de éxito
			setRespuestaText(""); // Limpiar el campo de texto después de enviar la respuesta
			fetchComentarios(); // Actualizar la lista de comentarios
			fetchPost();
		} catch (error) {
