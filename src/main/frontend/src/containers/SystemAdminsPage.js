...
function SystemAdminsPage() {
	...
	const handleDelete = async (id) => {
	 ...
	 const  handleActivate = async (id) => {
		try {
			await UpdateProfileService.unsuspendAyudante(id);
			fetchUsers();
			alert("Ayudante dado de alta satisfactoriamente");
		} catch (error) {
								...
								<CardActions>
									{user.activo ? (
										...
										<Button
											...
											onClick={() => handleActivate(user.id)} // Asumiendo que tienes una función handleActivate
										>
											Dar de alta
										</Button>
										)}
