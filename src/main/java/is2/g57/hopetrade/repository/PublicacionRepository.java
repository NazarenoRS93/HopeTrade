...
public interface PublicacionRepository extends JpaRepository<Publicacion, Integer> {
    @Query("SELECT p FROM Publicacion p WHERE p.user.id = :userID")
    List<Publicacion> findAllByUserID(Long userID);

    @Query("SELECT p FROM Publicacion p WHERE p.user.id = :userID AND p.state = :state")
    ...
    @Query("SELECT p FROM Publicacion p WHERE p.user.id = :userID AND (p.state.nombre = :statename1 OR p.state.nombre = :statename2)")
    ...
    @Query("SELECT p FROM Publicacion p WHERE p.user.id = :userID AND p.state.nombre = :statename")
